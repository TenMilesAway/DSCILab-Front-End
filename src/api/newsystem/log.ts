...
export const exportOperationLogExcelApi = (
  ...
  return http.download("/newlogs/operationLogs/excel", fileName, {
    params
  });
};

export const deleteOperationLogApi = (data: Array<number>) => {
  return http.request<ResponseData<void>>("delete", "/newlogs/operationLogs", {
    params: {
...
export const exportLoginLogExcelApi = (
  ...
  return http.download("/newlogs/loginLogs/excel", fileName, {
    params
  });
};

export const deleteLoginLogApi = (data: Array<number>) => {
  return http.request<ResponseData<void>>("delete", "/newlogs/loginLogs", {
    params: {
      // 需要将数组转换为字符串  否则Axios会将参数变成 noticeIds[0]:1  noticeIds[1]:2 这种格式，后端接收参数不成功
      ids: data.toString()
    }
